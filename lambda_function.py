...
def Performance(frame):
 ...
 #image_pts=np.matrix('shape.part(17).x, shape.part(17).y;shape.part(21).x, shape.part(21).y;shape.part(22).x, shape.part(22).y;shape.part(26).x, shape.part(26).y;shape.part(36).x, shape.part(36).y;shape.part(39).x, shape.part(39).y;shape.part(42).x, shape.part(42).y;shape.part(45).x, shape.part(45).y;shape.part(31).x, shape.part(31).y;shape.part(35).x, shape.part(35).y;shape.part(48).x, shape.part(48).y;shape.part(54).x, shape.part(54).y;shape.part(57).x, shape.part(57).y;shape.part(8).x, shape.part(8).y')      
  ...
  #euler_angles = cv.DecomposeProjectionMatrix(projMatrix=rmat, cameraMatrix=camera_matrix, rotMatrix, transVect, rotMatrX=None, rotMatrY=None, rotMatrZ=None)
    ...
    if euler_angle[1]>=20 and count1==2:
     #cv2.imwrite(sys.argv[1] + '/left.jpg', frame)
     EyelidR1=shape[27,0]
     EyelidR2=shape[27,1]
     EyelidR3=shape[0,0]
     EyelidR4=shape[0,1]
     EyelidR5=shape[16,0]
     EyelidR6=shape[16,1]
     zpos = (float)(EyelidR5-EyelidR3)
     tx = (float)((EyelidR1-240)*-0.9)
     ty = (float)((EyelidR2-240)*-1)
     rz = (float)(euler_angle[1]*1.1)
     rx = (float)(euler_angle[2]*-0.7)
     ry = (float)(euler_angle[0]-90)
     facedata['left'] = {'tx':tx,'ty':ty,'rx':rx,'ry':ry,'rz':rz,'zpos':zpos}
     renderviews+=1
     count1+=1;    
    elif euler_angle[1]>20 and count1<2:
     count1+=1;     
    elif euler_angle[1]<=-30 and count2==2:
     #cv2.imwrite(sys.argv[1] + '/right.jpg', frame)
     EyelidR1=shape[27,0]
     EyelidR2=shape[27,1]
     EyelidR3=shape[0,0]
     EyelidR4=shape[0,1]
     ...
     EyelidR6=shape[16,1]
     zpos = (float)(EyelidR5-EyelidR3)
     tx = (float)((EyelidR1-240)*-1)
     ty = (float)((EyelidR2-240)*-1)
     rz = (float)(euler_angle[1]*0.7)
     rx = (float)(euler_angle[2]*-0.7)
     ry = (float)(euler_angle[0]-90)
     facedata['right'] = {'tx':tx,'ty':ty,'rx':rx,'ry':ry,'rz':rz,'zpos':zpos}
     renderviews+=1
     count2+=1;
    elif euler_angle[1]<-15 and count2<2:
     count2+=1;     
    elif euler_angle[1]>-5 and euler_angle[1]<5 and count3==3:
     #cv2.imwrite(sys.argv[1] + '/center.jpg', frame)
     EyelidR1=shape[27,0]
     EyelidR2=shape[27,1]
     EyelidR3=shape[0,0]
     EyelidR4=shape[0,1]
     ...
     tx = (float)((EyelidR1-240)*-1)
     ty = (float)((EyelidR2-240)*-1)
     rz = (float)(euler_angle[1]+4)
     rx = (float)(euler_angle[2]*-1)
     ry = (float)(euler_angle[0]-90)
     facedata['center'] = {'tx':tx,'ty':ty,'rx':rx,'ry':ry,'rz':rz,'zpos':zpos}
     renderviews+=1
     count3=11;
    elif euler_angle[1]>-5 and euler_angle[1]<5 and count3<3:
     count3+=1;
...
def lambda_handler(event, context):
	...
	with open("/tmp/"+filename+".json", "w") as write_file:
		json.dump(facedata, write_file)
	s31.upload_file("/tmp/"+filename+".json",'jsonrecieve',filename+".json")
	call('rm -rf /tmp/*', shell=True)
	del(facedata)
	del(renderviews)
	del(count1)
